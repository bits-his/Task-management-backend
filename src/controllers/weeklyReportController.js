...
LEFT JOIN task_form t 
    ON wr.report_date = t.submitted_date 
    AND t.assigned_to = u.id
    AND t.status IN ('completed', 'underReview')
LEFT JOIN excuses e 
    ON wr.report_date = e.excuse_day 
    AND e.create_by = u.id
WHERE u.startups IS NOT NULL
GROUP BY u.id, wr.report_date, wr.week_start
ORDER BY wr.week_start DESC, wr.report_date ASC, u.id;
`,
